...
| 14 | WI-06 PDF renderer (follow-up) | ✅ Done — this turn. `services/pdf/safework_nsw.js` 7-section record-copy renderer + `?format=pdf` branch on `GET /reports/safework-nsw/:incidentId` + FE Download-PDF button on `SafeworkNswCardRows`. Government-document styling, no logo impersonation, footer carries verbatim s.38 submission channels (phone 13 10 50 + online portal) + "not a substitute" disclaimer. Lookup tables provide verbatim Act labels at render time. 43-assertion `wi06-pdf-e2e.sh`. Drive-by fix on the chunk-11 `listNotificationsForOrg` ambiguous-column bug. |
...
### WI-06: SafeWork NSW notification — engine + tables + routes + FE + PDF (WHS Act 2011 (NSW) ss.35–39) — ✅ DONE

Shipped 2026-05-12 across two commits. Verbatim s.35–s.39 text extracted from `docs/regulatory-sources/safework-nsw/whs-act-2011-nsw.pdf` (current version for 1 March 2026 to date) and owner-approved before any code was written. Engine + tables + routes + FE landed in `1a02ed6` (chunk 11); PDF record-copy renderer landed in chunk 14 (this turn).
...
**PDF renderer** (chunk 14, this turn):
- **`server/services/pdf/safework_nsw.js`** — US-Letter portrait, 7-section record-copy. Header title is "Notifiable Incident Record" with the subtitle "Internal record copy — Work Health and Safety Act 2011 (NSW), Part 3"; right-aligned reference column carries the NSW number + event date. Sections: (1) Notifying entity (PCBU), (2) s.35 categories with X-mark checkboxes, (3) s.36 sub-categories with verbatim labels + section refs pulled from `listSeriousInjuryTypes()`, (4) s.37 sub-categories ditto from `listDangerousIncidentTypes()`, (5) Incident narrative, (6) s.39 site preservation, (7) s.38 notification log (phone + regulator-requested-written + written-submitted). When `excluded_mines_petroleum=1`, a "Determination — Mines & Petroleum exclusion" block renders citing s.38(8)/s.39(4). Footer is the s.38 disclaimer: "Notifiable incidents are reported to SafeWork NSW by telephone (13 10 50) or online portal (notifyform.safework.nsw.gov.au). This document is the organisation's internal record copy of the notification and is not a substitute for that submission." Reuses the WI-09 manual-pagination pattern (`margins.bottom: 0`, `lineBreak: false`, `nextPageIfNeeded()` guard). **Government-document styling, NO SafeWork NSW logo, NO regulator-form impersonation** — per owner standing instruction.
- **Route** — `GET /reports/safework-nsw/:incidentId?format=pdf` branch added to the existing JSON endpoint. Loads the notification row + incident + site + primary `affected_persons` row + both lookup tables + the user names for `phone_notified_by` / `written_submitted_by`, calls the renderer, writes a `safework_nsw_pdf_downloaded` activity_log entry. JSON branch unchanged.
- **New audit verb** — `safework_nsw_pdf_downloaded` (entity_type='incident', metadata={nsw_notification_id, nsw_number, incident_id}).
- **FE** — `downloadSafeworkNswPdf(incidentId)` blob helper added to `client/src/api/safework_nsw.js`. `IncidentDetail.jsx` gains a `downloadingNsw` state + an inline auth-fetch + blob-`<a>` downloader (matching the WI-03/WI-09 button pattern). `SafeworkNswCardRows` gains a "Record copy" row at the bottom (renders even under the M&P carve-out so the determination record can be downloaded).
- **Drive-by fix** — `listNotificationsForOrg` had an ambiguous `org_id` column after the incidents/sites joins (chunk-11 bug). Qualified all column refs to `n.*`.
- **E2E** — `server/scripts/wi06-pdf-e2e.sh` (43 assertions). Sections: A seed (NSW-classified incident with multi-category s.36 + s.37 sub-categories), B happy-path PDF (200 + content-type + 16 verbatim-label/footer assertions), C activity_log + WI-C hash chain, D negatives (acme 403 + riddor 403 + unknown 404 + missing-notification 404), E Mines & Petroleum carve-out renders the "NOT NOTIFIABLE" determination, F phone-notified + written-submitted values surface in the PDF after lifecycle writes.
...
- PCBU "address" + "trading name" + "worker count" fields per original WI-06 spec — deferred (current PCBU capture is name + ABN + ANZSIC; PDF prints whatever is captured).
- Organisation logo embedding — `organizations.logo_path` does not exist today; renderer ready to accept `payload.orgLogoPath` when added.
